...
class FullPlanetariumTests(APITestCase):
    def setUp(self):
        self.user = User.objects.create_user(
            email="user@example.com", password="password123"
        )
    ...
    def test_filter_astronomy_show_by_description(self):
        url = reverse("planetarium:astronomyshow-list") + "?title=stars"
        self.client.force_authenticate(user=self.user)
        res = self.client.get(url)
        self.assertEqual(res.status_code, status.HTTP_200_OK)
        self.assertTrue(any("Stars" in show["title"] for show in res.data["results"]))

    def test_filter_show_session_by_title_and_date(self):
        url = reverse("planetarium:show_session-list")
        params = f"?title=stars&show_time=2025-05-18"
        self.client.force_authenticate(user=self.user)
    ...
    def test_create_dome(self):
        ...
        data = {"name": "Secondary", "rows": 8, "seats_in_row": 12}
    ...
    def test_create_astronomy_show(self):
        ...
        data = {
            "title": "Black Holes",
            "description": "Mysterious objects",
            "show_theme": [self.theme.id],
        }
        res = self.client.post(url, data)
        self.assertEqual(res.status_code, status.HTTP_201_CREATED)

    def test_invalid_ticket_seat_number(self):
        self.client.force_authenticate(user=self.user)
        url = reverse("planetarium:reservation-list")
        data = {"tickets": [{"show_session": self.session.id, "row": 1, "seat": 999}]}
        res = self.client.post(url, data, format="json")
        self.assertEqual(res.status_code, status.HTTP_400_BAD_REQUEST)
        self.assertIn("seat", res.data["tickets"][0])
    ...
    def test_create_reservation_with_multiple_tickets(self):
        ...
        data = {
            "tickets": [
                ...
                {"show_session": self.session.id, "row": 1, "seat": 2},
            ]
        }
        res = self.client.post(url, data, format="json")
        self.assertEqual(res.status_code, status.HTTP_201_CREATED)
